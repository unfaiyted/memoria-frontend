...
import { POST_PORTIS } from '$lib/api/clients'; // Removed unused GET_PORTIS
...
function createShortensStore() {
	...
	return {
		...
		async fetchShorten(url: string): Promise<Shorten | null> {
			...
			try {
				const response = await POST_PORTIS('/shorten/lookup', {
					body: {
						originalUrl: url,
		...
		async createShorten(shortenData: CreateShortenRequest): Promise<Shorten | null> {
			store.setLoading(true);
			try {
				const response = await POST_PORTIS('/shorten', {
					body: shortenData
				});
